const { group, indent, line, hardline, concat } = require('prettier').doc.builders;
const { EXPRESSION_NEEDED, isWhitespaceNode, indentWithHardline } = require('../util');
...
const p = (node, path, print) => {
  ...
  const isBodyEmpty =
    ...
    (node.body.expressions.length === 1 && isWhitespaceNode(node.body.expressions[0]));
