const { group, concat, line, hardline } = require('prettier').doc.builders;
const { printChildBlock, isNotExpression, STRING_NEEDS_QUOTES, GROUP_TOP_LEVEL_LOGICAL } = require('../util');
const { Node } = require('melody-types');

const shouldAvoidBreakBeforeClosing = (valueNode) =>
  Node.isObjectExpression(valueNode) || isNotExpression(valueNode) || Node.isArrayExpression(valueNode);
